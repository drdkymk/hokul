...
function ProblemBirimPage() {
  const [problemSelect, setProblemSelect] = useState([]);
  const [birimSelect, setBirimSelect] = useState([]);

  const [newRow, setNewRow] = useState({
    ProblemID: "", BirimID: "", EslesmeTarihi: ""
  });
  ...
  const addRow = () => {
    Axios.post("http://localhost:3001/api/ProblemBirim/", newRow)
      .then((response) => {
        if(response.data === ""){
          setShowToast(true);
          return;
  ...
  const updateRow = () => {
    Axios.put("http://localhost:3001/api/ProblemBirim", selectedRow)
      .then((response) => {
        if(response.data === ""){
          setShowToast(true);
          return;
  ...
  const deleteRow = () => {
    Axios.delete("http://localhost:3001/api/ProblemBirim", {
      data: {
        ProblemID: selectedRow["ProblemID"],
        BirimID: selectedRow["BirimID"],
      }
    })
      .then((response) => {
        setUpdate(!update);
  ...
  useEffect(() => {
    Axios.get("http://localhost:3001/api/ProblemBirim/")
      .then((response) => {
    ...
    <div className="App">
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          <tr>
            ...
            <td><Form.Control size="sm" type="text" value={searchRow["ProblemID"]} onChange={(e) => {
              let tempSearchRow = { ...searchRow };
              tempSearchRow["ProblemID"] = e.target.value;
              setSearchRow(tempSearchRow);
            }} />
            </td>
            <td><Form.Control size="sm" type="text" value={searchRow["BirimID"]} onChange={(e) => {
              let tempSearchRow = { ...searchRow };
              tempSearchRow["BirimID"] = e.target.value;
              setSearchRow(tempSearchRow);
            }} /></td>
            <td><Form.Control size="sm" type="text" value={searchRow["EslesmeTarihi"]} onChange={(e) => {
              let tempSearchRow = { ...searchRow };
              tempSearchRow["EslesmeTarihi"] = e.target.value;
              setSearchRow(tempSearchRow);
            }} /></td>
          </tr>
      ...
      <Offcanvas show={show} placement="end" onHide={togglePanel}>
        <Offcanvas.Header closeButton>
          <Offcanvas.Title>{isEdit ? "Problem-Birimi Düzenle" : "Yeni Problem-Birim"}</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
          ...
          </Alert>
          {
            isEdit ?
              <div><Form onSubmit={(e) => {
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Birim ID</Form.Label>
                  <Form.Select value={selectedRow["BirimID"]} onChange={(e) => {
                    let tempSelectedRow = { ...selectedRow };
                    tempSelectedRow["BirimID"] = e.target.value;
                    setSelectedRow(tempSelectedRow);
                  }} disabled>
                    <option value="" disabled>Seçiniz...</option>
                    {birimSelect.map((value, index) => {
                      return <option key={index} value={value.BirimKodu}>{value.BirimKodu + ": " + value.BirimAdi}</option>
                    })}
                  </Form.Select>
                </Form.Group>
                <Form.Group className="mb-3">
                  <Form.Label>Eşleşme Tarihi</Form.Label>
                  <Form.Control type="text" value={selectedRow["EslesmeTarihi"]} onChange={(e) => {
                    let tempSelectedRow = { ...selectedRow };
                    tempSelectedRow["EslesmeTarihi"] = e.target.value;
                    setSelectedRow(tempSelectedRow);
                  }} required />
                </Form.Group>
              ...
              : <div><Form onSubmit={(e) => {
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Birim ID</Form.Label>
                  <Form.Select value={newRow["BirimID"]} onChange={(e) => {
                    let tempNewRow = { ...newRow };
                    tempNewRow["BirimID"] = e.target.value;
                    setNewRow(tempNewRow);
                  }} required>
                    <option value="" disabled>Seçiniz...</option>
                    {birimSelect.map((value, index) => {
                ...
                {/* <Form.Group className="mb-3">
                  <Form.Label>Eşleşme Tarihi</Form.Label>
                  <Form.Control type="text" value={newRow["EslesmeTarihi"]} onChange={(e) => {
                    let tempNewRow = { ...newRow };
                    tempNewRow["EslesmeTarihi"] = e.target.value;
                    setNewRow(tempNewRow);
                  }} required />
                </Form.Group> */}
                <div style={{ textAlign: "center", justifyContent: "center" }}>
...
export default ProblemBirimPage;
