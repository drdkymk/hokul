...
function MudahalePage() {
  ...
  const addRow = () => {
    Axios.post("http://localhost:3001/api/Mudahale/", newRow)
      .then((response) => {
        if(response.data === ""){
          setShowToast(true);
          return;
  ...
  const updateRow = () => {
    Axios.put("http://localhost:3001/api/Mudahale", selectedRow)
      .then((response) => {
        if(response.data === ""){
          setShowToast(true);
          return;
  ...
  const deleteRow = () => {
    Axios.delete("http://localhost:3001/api/Mudahale", { data: { MudahaleID: selectedRow["MudahaleID"] } })
      .then((response) => {
        setUpdate(!update);
        togglePanel();
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          <tr>
            ...
            <td><Form.Control size="sm" type="text" value={searchRow["MudahaleAdi"]} onChange={(e) => {
              let tempSearchRow = { ...searchRow };
              tempSearchRow["MudahaleAdi"] = e.target.value;
              setSearchRow(tempSearchRow);
            }} /></td>
          </tr>
            ...
            isEdit ?
              <div><Form onSubmit={(e) => {
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control type="text" value={selectedRow["MudahaleID"]} onChange={(e) => {
                    let tempSelectedRow = { ...selectedRow };
                    tempSelectedRow["MudahaleID"] = e.target.value;
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Müdahale Adı</Form.Label>
                  <Form.Control type="text" value={selectedRow["MudahaleAdi"]} onChange={(e) => {
                    let tempSelectedRow = { ...selectedRow };
                    tempSelectedRow["MudahaleAdi"] = e.target.value;
                    setSelectedRow(tempSelectedRow);
                  }} required />
                </Form.Group>
              ...
              : <div><Form onSubmit={(e) => {
                ...
                {/* <Form.Group className="mb-3">
                  <Form.Label>Müdahale ID</Form.Label>
                  <Form.Control type="text" value={newRow["MudahaleID"]} onChange={(e) => {
                    let tempNewRow = { ...newRow };
                    tempNewRow["MudahaleID"] = e.target.value;
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Müdahale Adı</Form.Label>
                  <Form.Control type="text" value={newRow["MudahaleAdi"]} onChange={(e) => {
                    let tempNewRow = { ...newRow };
                    tempNewRow["MudahaleAdi"] = e.target.value;
                    setNewRow(tempNewRow);
                  }} required />
                </Form.Group>
                <div style={{ textAlign: "center", justifyContent: "center" }}>
